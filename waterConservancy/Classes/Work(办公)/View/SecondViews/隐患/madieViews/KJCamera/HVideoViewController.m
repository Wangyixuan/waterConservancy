...
- (void)viewDidLoad {
    ...
    self.backCenterX.constant = -(ScreenWidth/2/2)-image.size.width/2/2;
...
//        [Utility showProgressDialogText:@"视频处理中..."];
        ...
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:self.saveVideoUrl completionBlock:^(NSURL *assetURL, NSError *error) {
            NSLog(@"outputUrl:%@",weak_self.saveVideoUrl);
            [[NSFileManager defaultManager] removeItemAtURL:weak_self.saveVideoUrl error:nil];
            if (weak_self.lastBackgroundTaskIdentifier!= UIBackgroundTaskInvalid) {
                [[UIApplication sharedApplication] endBackgroundTask:weak_self.lastBackgroundTaskIdentifier];
            }
...
//                [Utility showAllTextDialog:KAppDelegate.window Text:@"保存视频到相册发生错误"];
                ...
                if (weak_self.takeBlock) {
                    weak_self.takeBlock(assetURL);
                }
                NSLog(@"成功保存视频到相簿.");
                [weak_self onCancelAction:nil];
            }
        }];
    } else {
    
        //照片
        self.takeImage = [self addText:self.takeImage text:nil];
        UIImageWriteToSavedPhotosAlbum(self.takeImage, nil, nil, nil);
...
- (void)changeLayout {
    ...
    self.afreshCenterX.constant = -(ScreenWidth/2/2);
    self.ensureCenterX.constant = ScreenWidth/2/2;
